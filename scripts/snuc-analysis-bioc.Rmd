...
if(file.exists(umi_file_name)){
  ...
  dat_umi = quminorm_matrix(dat_filtered, shape = getMode(sig_vec)) # Takes several minutes. In the paper, they use the mode.
...
dim(dat_umi) # 226608 x 576 for mRNA, 221988 x 576 for premRNA

if(dataset == "mrna"){
  row_ranges = rowRanges(se)
  genes = unlist(elementMetadata(row_ranges)$gene_id)
  dat = rowsum(dat_orig, group = genes, reorder = FALSE)
  dim(dat) # 60179 x 576 for mRNA
} else {
  ...
  # Note: 303 premRNA transcripts did not have corresponding mRNA transcripts
  transcripts_premrna = rownames(dat_orig)
  ...
  dat = rowsum(dat_orig, group = premrna_gene_matching_dt$genes, reorder = FALSE)
  dim(dat) # 60106 x 576
}

# Remove genes that have 0 counts for all cells
dat_gene = dat[!(rowSums(dat) == 0),] # 48942 x 576 for mRNA, 50501 x 576 for premRNA
