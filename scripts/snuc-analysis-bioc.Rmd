...
sum_counts_tb %>%
  ...
  ggplot(aes(x = transcripts_spliced, y = preandmrna_spliced, color = patient)) +
  geom_point(size = 0.5) +
...
sum_counts_tb %>%
  ...
  select(-pipeline) %>%
  pivot_wider(names_from = "source", values_from = "value") %>%
  ggplot(aes(x = transcripts_spliced, y = preandmrna_both, color = patient)) +
  geom_point(size = 0.5) +
  geom_abline(intercept = 0, slope = 1) +
  labs(title = "Number of reads mapped overall under different pipelines",
       subtitle = "Each point is a different cell") +
...
data.frame(PC2 = pc$x[,2], PC3 = pc$x[,3], patient = tumor_labels) %>%
  ggplot(aes(PC2, PC3, color = patient)) +
  geom_point(alpha = 0.5) +
  theme_bw()

data.frame(PC4 = pc$x[,4], PC5 = pc$x[,5], patient = tumor_labels) %>%
  ggplot(aes(PC4, PC5, color = patient)) +
  geom_point(alpha = 0.5) +
  theme_bw()
```
