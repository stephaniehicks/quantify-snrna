...
if(file.exists(umi_file_name)){
  ...
  dat_umi = quminorm_matrix(dat_filtered, shape = median(sig_vec)) # Takes several minutes (20~30 min). In the paper, they use the mode, but I will use the median since it is not clear what mode means for continuous values. The median for mRNA is . The median for pre-mRNA is 3.28. 
...
# Run approximate PCA (a few seconds)
tic("approx PCA")
