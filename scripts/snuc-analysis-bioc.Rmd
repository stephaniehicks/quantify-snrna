...
# Distribution of both pre-mRNA and mRNA

```{r}
summary(colSums(dat_filtered))

# Apply transformation
n = round(median(colSums(dat_filtered)))
dat_filtered = apply(dat_filtered, MARGIN = 2, function(x) x*n/sum(x))
...
```{r}
# Plot mean against variance
plot_meanvar(dat_filtered)
```

```{r}
# Plot P(X_i = 0) against average expression level
plot_prob(dat_filtered) +
  labs(title = "Both pre-mRNA and mRNA")
```
