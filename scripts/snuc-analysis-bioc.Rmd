...
First, I plot the mean and variance for every row (transcript). Under a poisson distribution,
...
I first compute the average expression for every row (x-axis) and the empirical $P(X_i=0)$, which is the probability that for a given gene $i$, the count is 0.
...
I add what $P(X_i=0)$ would be under the model assumptions of binomial or poisson, using parameters estimated from the data. In particular, for a $Binom(n_i, p_i)$, $n_i$ is the median number of total counts across cells and $p_i$ is the mean proportion of counts that were in gene $i$. 

```{r}
# Model P(X = 0) under Binomial
emp_props = rowMeans(dat_prem/colSums(dat_prem))
n = round(median(colSums(dat_prem)))
binom_probs_0 = dbinom(x = 0, size = n, p = emp_props)
# Model P(X = 0) under Poisson
poiss_probs_0 = dpois(x = 0, lambda = n*emp_props)

# Add to plot
plot_dt = plot_dt %>%
  mutate(binom_probs_0 = binom_probs_0,
         poiss_probs_0 = poiss_probs_0) %>%
  pivot_longer(-means, names_to = "model", values_to = "probs_0")
  
# Plot
plot_dt %>%
  ggplot(aes(x = log(means), y = log(probs_0), color = model)) +
  geom_point(alpha = 0.5) +
  labs(title = "pre-mRNA",
       x = "Average expression level E(X_i)",
       y = "P(X_i = 0)")
```
...
```{r}
# plot P(X=0) against the average expression level for every transcript
