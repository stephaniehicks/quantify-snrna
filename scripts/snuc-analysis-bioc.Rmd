...
counts = counts_bygene_preandmrna %>% select(-c("gene_id", "gene_type")) %>% as.matrix()
...
lib_counts = colSums(counts(sce_bygene_preandmrna))
prop_zero = colMeans(!counts(sce_bygene_preandmrna))

# By library counts
data.frame(PC1 = pc$x[,1], PC2 = pc$x[,2], lib_counts = lib_counts^(1/2)) %>%
  ggplot(aes(PC1, PC2, color = lib_counts)) +
  scale_colour_gradientn(colours = terrain.colors(10)) +
  geom_point(alpha = 0.5) +
  theme_bw()

data.frame(PC1 = pc$x[,1], PC3 = pc$x[,3], lib_counts = lib_counts^(1/2)) %>%
  ggplot(aes(PC1, PC3, color = lib_counts)) +
  scale_colour_gradientn(colours = terrain.colors(10)) +
  geom_point(alpha = 0.5) +
  theme_bw()

# By proportion of zeroes (genes with no reads mapped)
data.frame(PC1 = pc$x[,1], PC2 = pc$x[,2], prop_zero = prop_zero) %>%
  ggplot(aes(PC1, PC2, color = prop_zero)) +
  scale_colour_gradientn(colours = terrain.colors(10)) +
  geom_point(alpha = 0.5) +
  theme_bw()

data.frame(PC1 = pc$x[,1], PC3 = pc$x[,3], prop_zero = prop_zero) %>%
  ggplot(aes(PC1, PC3, color = prop_zero)) +
  scale_colour_gradientn(colours = terrain.colors(10)) +
  geom_point(alpha = 0.5) +
  theme_bw()
...
```{r}
# Turn counts by gene into sce
counts = counts_bygene_transcripts %>% select(-c("gene_id", "gene_type")) %>% as.matrix()
...
lib_counts = colSums(counts(sce_bygene_transcripts))
prop_zero = colMeans(!counts(sce_bygene_transcripts))
