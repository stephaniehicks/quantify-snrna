...
```{r}
sce_preandmrna = readRDS(here(paste0("salmon_quants_preandmrna"), "sce_preandmrna.rds"))
sce_transcripts = readRDS(here(paste0("salmon_quants_transcripts"), "sce_transcripts.rds"))
```
...
sum_counts_tb %>%
  ...
  ggplot(aes(x = transcripts_spliced, y = preandmrna_spliced)) +
  geom_point(size = 0.5) +
  geom_abline(intercept = 0, slope = 1) +
  labs(title = "Number of reads mapped to spliced transcripts under different pipelines",
       subtitle = "Each point is a different sample") +
  theme_bw()
...
sum_counts_tb %>%
  select(-pipeline) %>%
  pivot_wider(names_from = "source", values_from = "value") %>%
  ggplot(aes(x = transcripts_spliced, y = preandmrna_both)) +
  geom_point(size = 0.5) +
  geom_abline(intercept = 0, slope = 1) +
  labs(title = "Number of reads mapped overall under different pipelines",
       subtitle = "Each point is a different sample") +
...
dat_prem_normalized = prop.table(dat_prem, 2)
dat_mrna_normalized = prop.table(dat_mrna, 2)
```

### Log-log counts

```{r}
dat_prem_rowmeans = tibble(genes = rownames(dat_prem_normalized),
                           mean_prem = rowMeans(dat_prem_normalized))
dat_mrna_rowmeans = tibble(genes = rownames(dat_mrna_normalized),
                           mean_mrna = rowMeans(dat_mrna_normalized))
...
```{r}
dat_prem_subset = as_tibble(dat_prem_normalized[, 1:5]) %>%
  rename_all(function(x) paste0(x, "_premrna")) %>%
  mutate(genes = rownames(dat_prem_normalized))
dat_mrna_subset = as_tibble(dat_mrna_normalized[, 1:5]) %>%
  rename_all(function(x) paste0(x, "_mrna")) %>%
  mutate(genes = rownames(dat_mrna_normalized))
