...
data.frame(PC1 = pc$x[,1], PC3 = pc$x[,3], lib_size = lib_size) %>%
  ggplot(aes(PC1, PC3, color = lib_size)) +
  scale_colour_gradientn(colours = terrain.colors(10), trans = "sqrt") +
  geom_point(alpha = 0.5) +
  theme_bw()
...
data.frame(PC1 = pc$x[,1], PC3 = pc$x[,3], prop_mapped = prop_mapped) %>%
  ggplot(aes(PC1, PC3, color = prop_mapped)) +
  scale_colour_gradientn(colours = terrain.colors(10)) +
  geom_point(alpha = 0.5) +
  theme_bw()
...
# K-means clustering
set.seed(1)
clust.kmeans <- kmeans(reducedDim(sce_bygene_preandmrna, "PCA"), centers = 5)
...
# K-means clustering
set.seed(1)
clust.kmeans <- kmeans(reducedDim(sce_bygene_transcripts, "PCA"), centers = 5)
