...
For the mRNA only index pipeline, we already downloaded the FASTA file from GENCODE, so we just need to unzip it (`gencode.v32.transcripts.fa`). 

Note: Unzipping is technically not really necessary, since we will zip it into `gentrome_transcripts.fa.gz` later on anyway, but this will be consistent with the file formats in the other pipelines.
...
write.table(df, file = here("salmon_files", "gencode.v32.annotation.tx2gene.txt"), 
...
cat gencode.v32.transcripts.fa GRCh38.primary_assembly.genome.fa.gz > gentrome_transcripts.fa.gz
```
...
tx <- extractTxSeqs(gtf = gtf_file, genome = genome, type = "spliced") # you might want to use the transcripts FASTA directly here?
...
cat gencode.v32.preandmrna.fa GRCh38.primary_assembly.genome.fa.gz > gentrome_preandmrna.fa.gz
```
...
The `-t` argument is the decoy sequence. 
