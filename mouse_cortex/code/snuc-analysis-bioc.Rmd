...
plot_dt = tibble(chi_obs = sort(chi_obs_pois),
                 chi_quantile = qchisq(p = (1:length(chi_obs))/length(chi_obs), df = ncol(counts_sub_cluster_scaled) - 1),
...
```{r}
# Negative binomial
chi_obs_nb = p_chisq_test_2(counts_sub_cluster_scaled, distribution = "nb")
plot_dt = tibble(chi_obs = sort(chi_obs_nb),
                 chi_quantile = qchisq(p = (1:length(chi_obs))/length(chi_obs), df = ncol(counts_sub_cluster_scaled) - 1),
                 percentile_color = case_when(chi_obs > quantile(chi_obs, 0.995) ~ "> 99.5",
                                              chi_obs > quantile(chi_obs, 0.95) ~ "> 95",
                                              chi_obs > quantile(chi_obs, 0.90) ~ "> 90",
