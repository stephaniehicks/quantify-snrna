...
p = genes_rowmeans_tb %>%
  ...
  # filter(cell_type %in% c("Excitatory neuron", "Microglia", "Astrocyte")) %>%
  filter(cortex == "cortex1") %>%
  ggplot(aes(x = gc, y = transcripts, color = cell_type)) +
  # facet_wrap(cortex ~ .) +
  # geom_point(alpha = 0.01) +
  geom_smooth() + 
  scale_x_log10() +
...
p = genes_rowmeans_tb %>%
  ...
  # filter(cell_type %in% c("Excitatory neuron", "Microglia", "Astrocyte")) %>%
  filter(cortex == "cortex1") %>%
  ggplot(aes(x = length_biomart, y = transcripts, color = cell_type)) +
  # facet_wrap(cortex ~ .) +
  # geom_point(alpha = 0.01) +
  geom_smooth() + 
  scale_x_log10() +
...
whGenes = names(rowMeans(counts_sub_sweep) >= 10)
plot_dt %>%
  ...
  ggplot(aes(x = M, y = A, color = length_label)) +
