...
tmp %>%
  ...
  ungroup() %>%
  ggplot(aes(x = bin, y = median, color = source, fill = source)) + 
...
tmp %>%
  group_by(bin_transcript, source) %>%
  filter(count > 0) %>%
  summarize(boxplot = list(setNames(boxplot.stats(count)$stats,
                                    c('lower_whisker','lower_hinge','median','upper_hinge','upper_whisker')))) %>%
  unnest_wider(boxplot) %>%
  ungroup() %>%
  ggplot(aes(x = bin_transcript, y = median, color = source, fill = source)) + 
  geom_ribbon(aes(ymin = lower_hinge, ymax = upper_hinge), alpha = 0.1, linetype = "dashed") +
  geom_point() +
  geom_line() + 
  scale_y_log10() +
...
genes_rowsums_tb %>%
  ggplot(aes(x = length, y = transcript_length, color = 1-transcript_length/length)) + 
  geom_point(alpha = 0.05) + 
  scale_x_log10() + 
  scale_y_log10() +
  labs(x = "preandmrna length",
       y = "transcript length",
       color = "% Intronic Region") +
  theme_bw() +
  theme(legend.position = "top")
...
genes_rowsums_nA_tb %>%
  ...
  ggplot(aes(x = transcript_length, y = transcripts, color = preandmrna_max_nA == 0)) +
  geom_point(alpha = 0.01) +
  geom_smooth() + 
  scale_x_log10() +
  scale_y_log10() +
