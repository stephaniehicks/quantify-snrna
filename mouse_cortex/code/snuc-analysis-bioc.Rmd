...
```{r}
pipeline = "preandmrna"
res = readRDS(here(paste0("./mouse_cortex/output/de_ie_", pipeline, "_lfc_cqn_bc.rds")))
...
compare_glb_tb = genes_length_tb %>%
  mutate(endothelial_before = counts_sub[, 1],
    inhibitory_neuron_before = counts_sub[, 2],
    endothelial_after = counts_leftover[, 1],
    inhibitory_neuron_after = counts_leftover[, 2]) %>%
  pivot_longer(cols = endothelial_before:inhibitory_neuron_after,
               names_to = "type",
               values_to = "count")
