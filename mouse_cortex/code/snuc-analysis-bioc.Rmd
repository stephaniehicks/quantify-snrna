...
genes_rowmeans_tb %>%
  mutate(cell_group = case_when(cell_type %in% c("Excitatory neuron", "Inhibitory neuron") ~ 1,
                                ...
                                TRUE ~ 3)) %>%
  # filter(transcripts > 0) %>%
  ggplot(aes(x = transcripts_max_nA8, y = transcripts, color = cell_type)) +
  facet_wrap(cortex ~ cell_group, labeller = label_both) +
  # geom_point(alpha = 0.01) +
  geom_smooth() + 
  scale_x_log10() +
...
```
```{r}
cors_bias = genes_rowmeans_tb %>%
  group_by(cell_type, cortex) %>%
  mutate(across(where(is.numeric), log10)) %>%
  mutate(across(where(is.numeric), ~na_if(., -Inf))) %>%
  mutate(across(where(is.numeric), ~na_if(., Inf))) %>%
  summarize(cor_transcripts_transcript_length = cor(transcripts, transcript_length, use = "pairwise.complete.obs"),
            cor_preandmrna_transcript_length = cor(preandmrna, transcript_length, use = "pairwise.complete.obs"),
            cor_transcripts_length = cor(transcripts, length, use = "pairwise.complete.obs"),
            cor_preandmrna_length = cor(preandmrna, length, use = "pairwise.complete.obs"),
            cor_transcripts_transcripts_max_nA8 = cor(transcripts, transcripts_max_nA8, use = "pairwise.complete.obs"),
            cor_preandmrna_transcripts_max_nA8 = cor(preandmrna, transcripts_max_nA8, use = "pairwise.complete.obs"),
            cor_transcripts_preandmrna_max_nA8 = cor(transcripts, preandmrna_max_nA8, use = "pairwise.complete.obs"),
            cor_preandmrna_preandmrna_max_nA8 = cor(preandmrna, preandmrna_max_nA8, use = "pairwise.complete.obs")) %>%
  pivot_longer(cols = starts_with("cor_"), names_to = "variables", values_to = "correlation")

cors_bias %>%
  ggplot(aes(x = cell_type, y = correlation)) +
