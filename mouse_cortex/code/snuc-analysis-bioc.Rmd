...
celltype_markers_length_tb %>%
  filter(cell_type %in% c("Neurons", "Endothelial cells")) %>%
...
```{r}
# downsample by marker gene density
down_sample = function(x, p){
  i = x[1]
  prob = p[i]
  if(is.na(prob)) return(x)
  return(unlist(lapply(x[2:length(x)], function(y){rbinom(1, y, prob)})))
}
...
downsample_by_density = function(m, density_est, lengths){
  ...
  matrix_1 = apply(matrix_1, 1, down_sample, p = p1)
  ...
  matrix_2 = apply(matrix_2, 1, down_sample, p = p2)
...
counts_sub = downsample_by_density(m = counts_sub, density_est = density_est)
