...
```{r}
pipeline = "preandmrna"
res_1 = readRDS(here(paste0("./mouse_cortex/output/de_ea_", pipeline, "_lfc.rds")))
res_2 = readRDS(here(paste0("./mouse_cortex/output/de_ea_", pipeline, "_lfc_cqn.rds")))
res_3 = readRDS(here(paste0("./mouse_cortex/output/de_ea_", pipeline, "_lfc_cqn_bc.rds")))
...
label_res = function(res, pairwise = FALSE){
  ...
  if(pairwise){ # Use pairwise differential genes only
    celltype_markers = all.markers[unique(colData(sce_ls[["preandmrna"]])$singleR_labels_pruned)]
    # singler_marker_genes_1 = celltype_markers$Neurons$`Endothelial cells`
    # singler_marker_genes_2 = celltype_markers$`Endothelial cells`$Neurons
    singler_marker_genes_1 = celltype_markers$Neurons$`Astrocytes`
    singler_marker_genes_2 = celltype_markers$`Astrocytes`$Neurons
    ...
    singler_marker_genes_2 = celltype_markers_length_tb %>%
      filter(cell_type %in% c("Astrocytes")) %>% # need to change this for different cell types ("Astrocytes" or "Endothelial cells")
      pull(gene)
  }
  ...
  res = res %>%
    mutate(is_marker_gene = ifelse((gene %in% singler_marker_genes_1 &
                                   ...
                                   ifelse((gene %in% singler_marker_genes_1), "Neurons", 
                                          ifelse((gene %in% singler_marker_genes_2), "Astrocytes", "None"))),
...
get_prop = function(res, label_normalization){
  res = res %>%
    arrange(padj) %>%
    mutate(n_marker_genes_1 = cumsum(is_marker_gene == "Astrocytes"),
    ...
    select(gene, padj, n, prop_marker_genes_1, prop_marker_genes_2, label) %>%
...
res_prop %>%
  ...
  geom_line() +
  geom_point() + 
  facet_wrap(~ marker_genes_celltype, nrow = 2, scales = "free_y") +
  xlim(c(0, 5000)) +
  theme_bw()
```
