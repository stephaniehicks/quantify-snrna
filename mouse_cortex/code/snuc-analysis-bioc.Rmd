...
plotReducedDim(sce_ls[["preandmrna"]], "PCA", colour_by = "ding_labels", point_size = 1)
```
...
sum(p_values_pois < 0.05/length(p_values_pois), na.rm = T)/length(p_values_pois)
```
...
sum(p_values_nb < 0.05/length(p_values_nb), na.rm = T)/length(p_values_nb)
```
