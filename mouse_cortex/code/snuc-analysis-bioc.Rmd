...
summ = gene_sum_tb %>% 
  group_by(pipeline, cortex) %>% 
  summarize(median = median(processed_pseudogene))

# Plot
gene_sum_tb %>%
  ggplot(aes(x = pipeline, y = processed_pseudogene)) + 
  geom_boxplot() +
...
PCA done in `save-sce.R`.

```{r}
# PCA colored by provided attribute
plotReducedDim(sce_ls[["transcripts"]], "PCA", colour_by = "lib_size")
```
...
# Plot
mean_var_tb %>%
  filter(var_type %in% c("var_emp")) %>%
  ggplot(aes(x = mean_emp, y = var_value, color = var_type)) + 
  geom_point(alpha = 0.1) + 
  geom_line(data = mean_var_tb %>% filter(var_type %in% c("var_pois", "var_nb_v1", "var_binom")),
            aes(x = mean_emp, y = var_value, color = var_type)) +
