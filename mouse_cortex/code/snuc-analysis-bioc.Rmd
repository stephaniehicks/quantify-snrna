...
print(p)
# saveRDS(p, here("./mouse_cortex/plots/gene_length_bias_transcripts_cortex_1.rds"))
```
...
```{r}
transcripts_nA8 = readRDS(here("mouse_cortex", "salmon_files", "transcripts_nA8.rds"))
transcripts_nA8 = transcripts_nA8 %>%
  group_by(gene_id) %>%
  summarize(transcripts_max_nA8 = max(n))
transcripts_nA8 %>%
  ggplot(aes(x = transcripts_max_nA8)) +
  geom_histogram() +
  # scale_x_log10() +
  theme_bw()

preandmrna_nA8 = readRDS(here("mouse_cortex", "salmon_files", "preandmrna_nA8.rds"))
preandmrna_nA8 = preandmrna_nA8 %>%
  group_by(gene_id) %>%
  summarize(preandmrna_max_nA8 = max(n))
preandmrna_nA8 %>%
  ggplot(aes(x = preandmrna_max_nA8)) +
  geom_histogram() +
  # scale_x_log10() +
  theme_bw()
```
...
```{r}
genes_rowmeans_tb = genes_rowmeans_tb %>%
  left_join(., transcripts_nA8, by = c("gene" = "gene_id")) %>%
  left_join(., preandmrna_nA8, by = c("gene" = "gene_id"))
```
...
genes_rowmeans_tb %>%
  ...
  ggplot(aes(x = preandmrna_max_nA8, y = preandmrna, color = cell_type)) +
...
r2_bias = genes_rowmeans_tb %>%
  ...
  summarize(r2_transcripts_transcript_length = cor(transcripts, transcript_length, use = "pairwise.complete.obs")^2,
            ...
            r2_transcripts_transcripts_max_nA8 = cor(transcripts, transcripts_max_nA8, use = "pairwise.complete.obs")^2,
            r2_preandmrna_preandmrna_max_nA8 = cor(preandmrna, preandmrna_max_nA8, use = "pairwise.complete.obs")^2) %>%
...
genes_rowmeans_tb %>%
  ggplot(aes(x = transcripts_max_nA8, y = preandmrna_max_nA8)) + 
...
genes_rowmeans_tb %>%
  ggplot(aes(x = preandmrna_max_nA8, y = length)) + 
  geom_point(alpha = 0.5) +
  scale_x_log10() +
  scale_y_log10() +
  theme_bw()
...
```{r}
pipeline = "preandmrna"
res = readRDS(here(paste0("./mouse_cortex/output/de_ea_", pipeline, "_lfc_cqn.rds")))
...
p = res %>%
  ...
  theme_bw()
print(p)
# saveRDS(p, here(paste0("./mouse_cortex/plots/de_length_bias_", pipeline, "_ea.rds")))

# Density histograms for all genes (orange) vs differential genes (blue)
p = res %>%
  ggplot(aes(x = length, y = ..density..)) +
  geom_histogram(fill = "#E69F00", alpha = 0.5) +
  geom_histogram(data = res %>% filter(padj < 0.05), aes(x = length, y = ..density..), fill = "#56B4E9", alpha = 0.5) +
  scale_x_log10() +
  labs(x = "Gene length", y = "Density") +
  theme_bw()
print(p)
# saveRDS(p, here(paste0("./mouse_cortex/plots/de_length_bias_hist_", pipeline, "_ea.rds")))
```
...
data(montgomery.subset) # counts
data(sizeFactors.subset) # size factors
data(uCovar)             # gene length and GC-content

cqn.subset <- cqn(montgomery.subset, lengths = uCovar$length,
                  x = uCovar$gccontent, sizeFactors = sizeFactors.subset,
                  verbose = TRUE)

RPKM.cqn <- cqn.subset$y + cqn.subset$offset # normalized expression values
...
```{r}
sce_sub = sce_ls[["preandmrna"]][, colData(sce_ls[["preandmrna"]])$ding_labels %in% c("Excitatory neuron", "Astrocyte")]
counts_sub = as.matrix(round(counts(sce_sub))) # original counts
seq_cqn = readRDS(here("./mouse_cortex/output/counts_eapreandmrna_cqn.rds")) # normalized counts

# Groups
grp1 = rownames(pData(seq_cqn))[pData(seq_cqn)$ding_labels == "Excitatory neuron"]
grp2 = rownames(pData(seq_cqn))[pData(seq_cqn)$ding_labels == "Astrocyte"]

# Compare
counts_sub_sweep = sweep(log2(counts_sub + 1), 2, log2(colData(sce_sub)$sizeFactor)) # divide by size factors
M_std = rowMeans(counts_sub_sweep[, grp1]) - rowMeans(counts_sub_sweep[, grp2])
A_std = rowMeans(counts_sub_sweep)

counts_cqn_sweep = sweep(log2(counts(seq_cqn) + 1), 2, log2(pData(seq_cqn)$sizeFactor)) # divide by size factors
M_cqn = rowMeans(counts_cqn_sweep[, grp1]) - rowMeans(counts_cqn_sweep[, grp2])
A_cqn = rowMeans(counts_cqn_sweep)

gene_lengths = rowData(sce_sub) %>%
  as_tibble() %>%
  select(start, end) %>%
  mutate(length = end - start,
...
plot_dt %>%
  filter(length > quantile(length, 0.9) | length < quantile(length, 0.1)) %>%
  ggplot(aes(x = M, y = A, color = length > quantile(length, 0.9))) +
  facet_wrap(~ data) +
  geom_point(alpha = 0.01) + 
...
```{r}
# Poisson (grouped)
chi_obs_pois = p_chisq_test_2_grouped(counts_sub_cluster_scaled, distribution = "poisson")
...
p = plot_dt %>%
  ...
  theme_bw()
print(p)
# saveRDS(p, here("./mouse_cortex/plots/qq_plot_poisson_astrocyte_cortex_1.rds"))
```
