...
genes_rowsums_tb = genes_rowsums_tb %>%
  mutate(median_exp = median(transcripts, preandmrna, introncollapse, intronseparate),
         preandmrna_percent_inc = (preandmrna - transcripts)/transcripts)

# Median % increase in each bin
genes_rowsums_tb = genes_rowsums_tb %>%
  mutate(bin = ntile(median_exp, 10))
genes_rowsums_tb %>%
  group_by(bin) %>%
...
```{r}
pipeline = "preandmrna"
res = readRDS(here(paste0("./mouse_cortex/output/de_", pipeline, ".rds")))
