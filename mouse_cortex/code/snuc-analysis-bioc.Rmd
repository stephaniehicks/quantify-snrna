...
for(i in seq_along(sce_ls)){
  ...
  for(cell_type in ding_cell_types){
    ...
    cells = colData(sce) %>% 
      ...
      rownames()
    cells = intersect(cells, common_cells)
    
    m = counts(sce[, cells])
    genes_rowsums = rowSums(m)
    tmp_tb_ls[[cell_type]] = tibble(gene = names(genes_rowsums),
                                    !!pipeline_name := genes_rowsums,
                                    cell_type = cell_type)
  }
  ...
  if(i == 1){
    genes_rowsums_tb = tmp_tb
  } else {
    genes_rowsums_tb = genes_rowsums_tb %>%
      left_join(., tmp_tb, by = c("gene", "cell_type"))
  }
}
...
genes_rowsums_tb %>%
  ggplot(aes(x = transcript_length, y = preandmrna, color = cell_type)) +
  # geom_point() + 
  geom_smooth() + 
  scale_x_log10() +
  scale_y_log10() +
