...
sce_sub = sce_ls[["preandmrna"]][, colData(sce_ls[["preandmrna"]])$ding_labels %in% c("Excitatory neuron", "Inhibitory neuron")]
...
```{r}
pipeline = "preandmrna"
res = readRDS(here(paste0("./mouse_cortex/output/de_", pipeline, "_norm.rds")))
...
res %>%
  ...
  geom_point(aes(color = log1p(preandmrna)), alpha = 0.05) +
  geom_smooth() +
...
# Manually adjusted p-values (BH)
res %>%
  filter(!is.na(preandmrna)) %>%
  mutate(padj_manual = p.adjust(pvalue, method = "BH")) %>%
  ggplot(aes(x = length, y = padj_manual)) + 
  geom_point(aes(color = log1p(preandmrna)), alpha = 0.05) +
  geom_smooth() +
  scale_colour_gradientn(colours = terrain.colors(10)) +
  scale_y_continuous(limits = c(0, 1)) +
...
for(i in 1:nrow(counts_sub_cluster_scaled)){
  ...
  prop5 = c(prop5, sum(f_hyp > 5)/length(f_hyp)) # Proportion of expected counts > 5
...
chiSquare.nb = rowSums((f_obs-f_hyp)^2)/apply(counts_sub_cluster_scaled, 1, nb_var) # chi-squared statistic for each gene
df = ncol(f_obs) - 1

plot_dt = tibble(chi_obs = sort(chiSquare.nb),
                 chi_quantile = qchisq(p = (1:length(chiSquare.nb))/length(chiSquare.nb),
                                       df = df),
                 percentile_color = case_when(chi_obs > quantile(chi_obs, 0.995) ~ "> 99.5",
                                        chi_obs > quantile(chi_obs, 0.95) ~ "> 95",
                                        chi_obs > quantile(chi_obs, 0.90) ~ "> 90",
