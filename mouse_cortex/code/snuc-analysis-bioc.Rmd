...
tmp = genes_rowsums_tb %>%
  ...
  pivot_longer(cols = ends_with("reads"),
               ...
               values_to = "count") 
p = tmp %>%
  ggplot(aes(x = as.factor(bin), y = count, color = source)) + 
  geom_boxplot() + 
  scale_y_log10() +
...
p = plot_dt %>%
  ...
  theme_bw()
print(p)
saveRDS(p, here(paste0("./mouse_cortex/plots/qq_plot_poisson_preandmrna_", snakecase::to_snake_case(cell_type), "_cortex_1.rds")))
```
