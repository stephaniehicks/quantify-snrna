...
label_res = function(res){
  # Label which genes are marker genes
  singler_marker_genes = celltype_markers_length_tb %>%
    filter(cell_type %in% c("Neurons", "Astrocytes")) %>% # need to change this for different cell types
    pull(gene)
  
  res = res %>%
    mutate(is_marker_gene = (gene %in% singler_marker_genes),
           is_differential_gene = padj < 0.05)
  return(res)
}

res_1 = label_res(res_1)
res_2 = label_res(res_2)
res_3 = label_res(res_3)
...
res_1 %>%
  ...
  facet_wrap(~ is_marker_gene.y, scales = "free", labeller = label_both) +
  geom_bar() +
  theme_bw() +
