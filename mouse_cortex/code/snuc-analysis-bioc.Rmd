...
```{r}
pipeline = "preandmrna"
res = readRDS(here(paste0("./mouse_cortex/output/de_ie_", pipeline, "_lfc.rds")))
...
p = res %>%
  ...
  geom_density(data = res %>% filter(padj < 0.05), aes(x = length, y = ..density..), fill = "#56B4E9", alpha = 0.5) +
  scale_x_log10() +
...
res_with_cqn = readRDS(here(paste0("./mouse_cortex/output/de_ie_", pipeline, "_lfc_cqn.rds")))
...
```{r}
sce_sub = sce_ls[["preandmrna"]][, colData(sce_ls[["preandmrna"]])$ding_labels %in% c("Inhibitory neuron", "Endothelial") & colData(sce_ls[["preandmrna"]])$cortex == "cortex2"]
...
# aggregated as mean of nonzero counts, then multiply all genes by some constant factor = number of cells
counts_sub = as.matrix(counts(sce_sub))
colnames(counts_sub) = colData(sce_sub)$ding_labels
tmp_rowmeans = list()
cell_types = unique(colnames(counts_sub))
for(ct in cell_types){
  tmp = counts_sub[, colnames(counts_sub) == ct]
  # tmp = counts_sub[1:5, 1:5]
  tmp[tmp == 0] = NA
  tmp_rowmeans[[ct]] = apply(tmp, 1, function(x) mean(x, na.rm = TRUE))*ncol(tmp)
}
counts_sub = do.call(cbind, tmp_rowmeans) 
colnames(counts_sub) = cell_types

# aggregated as rowsums
# counts_sub = t(rowsum(t(counts_sub), colnames(counts_sub)))

counts_sub = round(counts_sub)
counts_sub[is.nan(counts_sub)] = 0 # may be OK to fill in NaN with 0 depending on cQN treats zero counts (need to check)
...
tic()
set.seed(1)
counts_sub_cluster_scaled = Down_Sample_Matrix(round(counts_sub_cluster))
toc()
