...
# Gene rowsum across the 4 pipelines, where bin on the x-axis is by full preandmrna length
genes_rowsums_tb %>%
  ...
  geom_boxplot() + 
  scale_y_log10() + 
  labs(x = "Bin (by full gene length)") + 
  theme_bw() +
  theme(legend.position = "top")

# Length bias of intronic reads (preandmrna - transcripts) versus exonic reads (transcripts)
genes_rowsums_tb %>%
  mutate(intronic_reads = preandmrna - transcripts,
         exonic_reads = transcripts) %>%
  select(intronic_reads, exonic_reads, bin, bin_transcript) %>%
  pivot_longer(cols = ends_with("reads"),
               names_to = "source",
               values_to = "count") %>%
  ggplot(aes(x = as.factor(bin_transcript), y = count, color = source)) + 
  geom_boxplot() + 
  scale_y_log10() +
...
res_3 = readRDS(here(paste0("./mouse_cortex/output/de_ie_", "transcripts", "_lengthsforcepreandmrna_TRUE", "_lfc.rds")))
...
# res_prop_3 = get_prop(res_3, label_normalization = "transcripts_full_length")
res_prop = bind_rows(res_prop_1, res_prop_2)
