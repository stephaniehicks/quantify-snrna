...
pipeline = "preandmrna" # "transcripts" or "preandmrna"
...
inner_join(lib_sizes$transcripts, lib_sizes$preandmrna, by = "cell_barcode") %>%
  ...
  labs(x = "lib_size_transcripts", y = "lib_size_preandmrna") +
  theme_bw()
```
...
overlap = table(colData(sce)$singleR_labels_pruned, colData(sce)$ding_labels) %>%
  as.data.frame()
...
sse = c()
for(k in 1:10){
  clust.kmeans <- kmeans(reducedDim(sce, "PCA"), centers = k)
  sse = c(sse, clust.kmeans$tot.withinss)
}
plot(sse)

clust.kmeans <- kmeans(reducedDim(sce, "PCA"), centers = 6)
colData(sce)$labels_kmeans <- factor(clust.kmeans$cluster)
plotReducedDim(sce, "PCA", colour_by = "labels_kmeans")
```
...
colData(sce)$labels_gb <- factor(clust)
plotReducedDim(sce, "PCA", colour_by="labels_gb")
```
...
```{r}
# Subset to cluster
counts_sub = as.matrix(assay(sce, "counts")[, (colData(sce)$ding_labels == "Excitatory neuron" & !is.na(colData(sce)$ding_labels))])
...
First, I make all the column sums the same by scaling (is downsampling preferred?). The count in row $i$ and column $j$ is transformed as $x_{ij} = x_{ij}*\sum_i x_{ij}/median_j(\sum_i x_{ij})$. 

```{r pre-mRNA_distribution}
summary(colData(sce)$lib_size)

# Apply transformation
n = round(median(colData(sce)$lib_size))
counts_sub_scaled = apply(counts_sub, MARGIN = 2, function(x) x*n/sum(x))
...
# Plot p-values
tibble(distribution = c(rep("Poisson", length(p_values_pois)), rep("NB", length(p_values_nb))),
...
1. Taking log for sz? Why does offset require taking log?
...
poi_fit<-function(m,X=NULL,sz=NULL,maxit=100){
  ...
  if(is.null(X)){ #no covariates => closed form solution
    sz<-exp(sz)
    lam<-rowSums(m)/sum(sz) # each gene has an empirically estimated lambda
    mu<-outer(lam,sz) # outer product of lambda and lib size, each cell x gene has a mu
...
nb_fit<-function(m,X=NULL,sz=NULL){
  #neg binom
  if(is.null(sz)){ sz<-colMeans(m)} # Note: original code took log but that led to invalid xmin
  if(is.null(X)){
    f<-function(y){
...
# Poisson
fit_pois <- glmGamPoi::glm_gp(counts_sub, design = ~ 1, size_factors = FALSE, 
...
tmp_pois <- data.frame("mean_fit" = rowMeans(fit_pois$Mu), 
           ...
           "variance_emp" = genefilter::rowVars(counts_sub)) %>% 
...
# Negative binomial
fit_nb <- glmGamPoi::glm_gp(counts_sub, design = ~ 1, size_factors = FALSE, 
                         overdispersion = TRUE)
fit_nb
names(fit_nb)
