...
write.table(df %>% dplyr::select(transcript_id, gene_id), 
            file = here("salmon_files", "mouse", "gencode.vM25.annotation.tx2gene.mouse.txt"), 
            sep = "\t", quote = FALSE, row.names = FALSE, 
            col.names = FALSE)
saveRDS(df, file = here("salmon_files", "mouse", "gencode.vM25.annotation.tx2gene.mouse.rds"))
```
...
Biostrings::writeXStringSet(preandmrna, file = here("salmon_files", "gencode.vM25.preandmrna.mouse.fa"))
```
...
t2g <- readRDS(here("salmon_files", "gencode.vM25.annotation.tx2gene.mouse.rds"))
...
write.table(t2g %>% dplyr::select(transcript_id, gene_id), 
            file = here("salmon_files", "gencode.vM25.preandmrna.tx2gene.mouse.txt"), 
...
gzip gencode.vM25.preandmrna.mouse.fa # Note: This will take a while (~ 1 hour)
...
We also need to use the linkedTxome object to use `tximeta` properly, i.e. `rowRanges(se)` won't be `NULL` and tximeta will be able to match the transcripts to the genes for us. 
...
se = tximeta(coldata = data.frame(names = "SRR9169236",
                                  files = here("alevin_output", "alevin", "quants_mat.gz"),
                                  stringsAsFactors = FALSE),
             type = "alevin")
