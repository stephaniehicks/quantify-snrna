...
inner_join(lib_sizes$transcripts, lib_sizes$preandmrna, by = "cell_barcode") %>%
  ...
  pivot_longer(cols = c("preandmrna", "transcripts"), names_to = "pipeline") %>%
...
```{r}
# Comparison of singleR classification between pipelines
preandmrna_cd = as_tibble(tmp) %>%
  mutate(cell = rownames(tmp)) %>%
  dplyr::rename(singleR_labels_pruned_preandmrna = singleR_labels_pruned) %>%
  select(cell, singleR_labels_pruned_preandmrna)
transcripts_cd = as_tibble(colData(sce)) %>%
  mutate(cell = rownames(colData(sce))) %>%
  dplyr::rename(singleR_labels_pruned_transcripts = singleR_labels_pruned)

cd = inner_join(preandmrna_cd, transcripts_cd, by = "cell")
cd %>%
  ggplot(aes(x = singleR_labels_pruned_preandmrna, fill = singleR_labels_pruned_transcripts)) +
  geom_bar() +
  theme_bw() +
...
poi_fit<-function(m,X=NULL,sz=NULL,maxit=100){
  ...
  if(is.null(X)){ #no covariates => closed form solution
    sz<-exp(sz) # sz is library size / number of genes
