...
preandmrna_cd_tb = as_tibble(preandmrna_cd) %>%
  mutate(cell = rownames(preandmrna_cd)) %>%
  dplyr::rename(singleR_labels_pruned_preandmrna = singleR_labels_pruned,
                singleR_labels_preandmrna = singleR_labels)
transcripts_cd_tb = as_tibble(transcripts_cd) %>%
  mutate(cell = rownames(transcripts_cd)) %>%
  dplyr::rename(singleR_labels_pruned_transcripts = singleR_labels_pruned,
                singleR_labels_transcripts = singleR_labels)

cd = inner_join(preandmrna_cd_tb, transcripts_cd_tb, by = "cell")
cd %>%
  ggplot(aes(x = singleR_labels_preandmrna, fill = singleR_labels_transcripts)) +
  geom_bar() +
  theme_bw() +
  theme(axis.text.x = element_text(angle = 90, hjust = 1))

table(cd$singleR_labels_pruned_preandmrna, cd$singleR_labels_pruned_transcripts, useNA = "ifany")
```
...
tb = tb %>%
  mutate(ding_labels_match = case_when(ding_labels == "Astrocyte" ~ "Astrocytes",
                                       ...
                                       ding_labels == "Microglia" ~ "Microglia",
                                       ding_labels == "Oligodendrocyte" ~ "Oligodendrocytes",
                                       ding_labels == "OPC" ~ "OPC"))

tb = tb %>%
  mutate(non_matching_cells = (singleR_labels_pruned != ding_labels_match) | is.na(singleR_labels_pruned))

tb_misclassify = tb %>%
  filter(!is.na(ding_labels_match)) %>%
  filter(ding_labels_match != "OPC")

tb_misclassify %>%
  ggplot(aes(x = lib_size)) +
  geom_histogram(aes(fill = non_matching_cells)) +
  facet_wrap(~ ding_labels_match, nrow = 2) +
...
plotReducedDim(sce_sub, "PCA", colour_by = "labels_kmeans")
```
...
# Print BIC values (lower value is better fit)
tic()
mult_bic(m) # 5 sec
toc() 
