...
astrocyte_mg = unique(unlist(all.markers$Microglia))
...
astrocyte_cells = cd %>% filter(ding_labels == "Microglia") %>% pull(cell)
...
for(i in seq_along(sce_ls)){
  ...
  lib_size_gene_type = colSums(counts(sce_i)[intersect(rownames(counts(sce_i)), genes_in_type), astrocyte_cells])
  lib_size_gene_type_tb = tibble(cell_barcode = names(lib_size_gene_type),
                                 !!gene_type_name := lib_size_gene_type)
  lib_size_tb = lib_size_tb %>% 
    left_join(., lib_size_gene_type_tb, by = "cell_barcode")
