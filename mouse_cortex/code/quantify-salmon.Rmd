...
Combine the different SummarizedExperiment files into one SummarizedExperiment for each pipeline.
...
se_ls = list()
for(run_name in run_names[c(1,16)]){
  se_ls[run_name] = readRDS(here("mouse_cortex", "salmon_quants", paste0(pipeline, "_pipeline"), paste0("se_", run_name, ".rds")))
}

# Merge rds files together
counts = as.matrix(assay(se_ls[[1]], "counts"))[1:2, 1:2]
colnames(counts) = paste(colnames(counts), run_names[1], sep = "_") # Add run number to cell barcodes
for(i in 2:2){
  m2 = as.matrix(assay(se_ls[[2]], "counts"))[1:2, 1:2]
  colnames(m2) = paste(colnames(m2), run_names[i], sep = "_")
  counts = merge(counts, m2, by = "row.names", all = TRUE)
}

row.names(counts) = counts[, 1]
counts = counts[, 2:ncol(counts)]
tmp = SummarizedExperiment(assays = list(counts = counts))
```
