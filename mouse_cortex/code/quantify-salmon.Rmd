...
mean_var_tb = tibble(mean_emp = mean_emp,
                     ...
                     var_nb_v1 = mean_emp + mean_emp^2 * 1/phi, # overdispersion is same for every gene
                     var_nb_v2 = mean_emp + mean_emp^2 * fit_nb$overdispersions) %>% # overdispersion parameter is different for every gene
...
smoothScatter(mean_var_tb %>%
  filter(var_type %in% c("var_pois")) %>% select(mean_emp, var_value) %>% log10())
```
