...
counts_ls[[i+1]] = matrix(rnbinom(n = 347*21434, size = 63, mu = 0.1), ncol = 347, nrow  = 21434) # negative binomial
counts_ls[[i+2]] = matrix(rpois(n = 347*21434, lambda = 0.1), ncol = 347, nrow = 21434) # poisson 
...
counts_ls[[i+3]] = t(sapply(seq_len(length(means)), function(i) 
...
for(i in seq_along(counts_ls)){
  ...
  plot_dt_pois_ls[[i]] = tibble(chi_obs = sort(chi_obs_pois),
                                chi_quantile = qchisq(p = (1:length(chi_obs))/length(chi_obs), df = ncol(counts) - 1),
  ...
  plot_dt_nb_ls[[i]] = tibble(chi_obs = sort(chi_obs_nb),
                              chi_quantile = qchisq(p = (1:length(chi_obs))/length(chi_obs), df = ncol(counts) - 2),
                              percentile_color = case_when(chi_obs > quantile(chi_obs, 0.995) ~ "> 99.5",
                                                           chi_obs > quantile(chi_obs, 0.95) ~ "> 95",
                                                           chi_obs > quantile(chi_obs, 0.90) ~ "> 90",
                                                           T ~ "rest"),
