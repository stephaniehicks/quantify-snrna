...
# Poisson simulations
lambdas = c(0.1, 0.3, 0.5, 0.7, 1.0)
for(i in seq_along(lambdas)){
  counts_ls[[i+4]] = matrix(rpois(n = 347*21483, lambda = lambdas[i]), ncol = 347, nrow = 21483)
}
counts_ls[[10]] = t(sapply(seq_len(length(means)), function(i) 
...
for(i in seq_along(lambdas)){
  for(j in seq_along(phis)){
    counts_ls[[(i-1)*length(phis)+j+10]] = matrix(rnbinom(n = 347*21483, size = phis[j], mu = lambdas[i]), ncol = 347, nrow = 21483)
  }
}

counts_ls[[31]] = t(sapply(seq_len(length(means)), function(k) 
...
saveRDS(counts_ls, here("./scrna/counts_more_ls.rds"))
```
...
# Use minimum bin number for each chi-squared test (length(bins) + 1 - p)
df_pois = sapply(plot_dt_pois_ls, function(x) min(x$df))
df_nb = sapply(plot_dt_nb_ls, function(x) min(x$df))
```
...
# Chi-squared tests where every row has the same df
...
for(i in 1:length(plot_dt_pois_ls)){
  plot_dt_pois_ls[[i]] = plot_dt_pois_ls[[i]] %>%
    arrange(chi_square) %>%
    mutate(chi_quantile = qchisq(p = (1:length(chi_square))/length(chi_square), df = ncol(.) - 2),
