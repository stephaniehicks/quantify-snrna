...
counts_ls[[i+4+1]] = t(sapply(seq_len(length(means)), function(i) 
...
for(i in seq_along(lambdas)){
  for(j in seq_along(phis)){
    counts_ls[[(i-1)*length(phis)+j+11]] = matrix(rnbinom(n = 347*21483, size = phis[j], mu = lambdas[i]), ncol = 347, nrow = 21483)
  }
}

counts_ls[[32]] = t(sapply(seq_len(length(means)), function(k) 
...
for(i in 1:31){
  ...
  plot_dt_nb_ls[[i]] = readRDS(here(paste0("./scrna/output/plot_dt_nb_1_ls_", i, ".rds")))
