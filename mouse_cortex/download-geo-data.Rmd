...
We are going ot focus on the mouse cortex. 
...
```{r, eval=FALSE}
sra <- readr::read_csv(here("00-files", "SraRunInfo.csv"))
pdata <- dplyr::left_join(pdata, sra, by = "Experiment")

# save phenotype table
readr::write_csv(pdata, path = here("00-files", "pdata.csv"))
```

#### Create subsets of the table

```{r}
pdata <- readr::read_csv(file = here("00-files", "pdata.csv"))
...
write.table(pdata_10x$Run, file = here("files", "SRR_files_10x.txt"), 
            quote= FALSE,row.names = FALSE, col.names = FALSE)
write.table(pdata_10x$download_path, file = here("files", "SRR_paths_10x.txt"), 
            quote= FALSE,row.names = FALSE, col.names = FALSE)
```
...
Or run the shell script `download-geo-data.sh`.
